...
- **自动 AI 策展**：首次播放时自动调用 Kimi 挑选最匹配的歌单并重新组池；策展结果本地缓存 24 小时。
- **单屏交互**：所有操作在一屏内完成，点击情绪球播放/暂停，左右滑动切歌。
- **氛围着色**：AI 生成的标签会实时改变情绪球和背景粒子的颜色（如「happy」变暖金，「rain」变冷蓝）。
- **音源视觉反馈**：网易云播放与生成式音频拥有不同的 Orb 呼吸节奏、粒子速度和光晕强度。
...
- **多层缓存**：氛围文案、策展结果、歌单列表、URL 可用性均在本地缓存，重复访问秒开并减少服务端压力。
...
src/components
  ...
  MoodBackground.tsx             # 背景氛围渐变与粒子
  BackgroundFlow.tsx             # Canvas 流动连线背景
  MoodOutput.tsx                 # 氛围文案、播放控制、状态提示

src/hooks
  useAtmosphere.ts               # 单首歌曲氛围文案请求
  useCurate.ts                   # AI 策展请求与本地缓存
  useSongPool.ts                 # 网易云候选曲目管理
  useReducedMotion.ts            # 检测并响应 reduced-motion 偏好

src/lib
  atmosphere-cache.ts            # 氛围文案本地缓存
  atmosphere-colors.ts           # 氛围标签到颜色的映射
  generative-audio.ts            # Web Audio 生成式音频引擎
  logger.ts                      # 开发环境日志封装
  music-theory.ts                # 音阶和音乐理论工具
  netease.ts                     # 网易云 API 工具、客户端缓存与歌单目录
...
- 客户端缓存集中在 `netease.ts`（歌单/URL 可用性）和 `atmosphere-cache.ts`（氛围文案），失效时间见对应文件常量。
